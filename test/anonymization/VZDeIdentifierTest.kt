...
class VZDeIdentifierTest {
    private lateinit var vzDeIdentifier: DeIdentifier

    @Before
    fun setup() {
        vzDeIdentifier = DependencyBinder.vzDeIdentifier
    }

    @Test
    fun `should return empty string on blank string`() {
        val anonymisedText = vzDeIdentifier.getDeIdentifiedText("   ")
    ...
    fun `text should not contain digits any more`() {
        ...
        testData.forEach {
            val anonymisedText = vzDeIdentifier.getDeIdentifiedText(it)
        ...
        testData.forEach {
            ...
            if (namedEntities.isNotEmpty()) {
                ...
                namedEntities.forEach {
                    assertFalse {
                        (DependencyBinder.vzDeIdentifier as? VZDeIdentifierImpl)?.sensibleNerTypes?.contains(it.value) == true
